      ...
      <div className="px-8">
            ...
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link
                  ...
                  className="inline-flex items-center gap-2 bg-black dark:bg-white text-white dark:text-black px-6 py-2.5 rounded-md hover:bg-black/90 dark:hover:bg-white/90 transition-colors text-small tracking-wide"
                >
        ...
        <div className="max-w-[1200px] mx-auto container-padding py-6 md:py-8 lg:py-10">
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div className="bg-white dark:bg-[#0a0a0a] border border-black/5 dark:border-white/10 rounded-lg p-10">
              <h3 className="text-small tracking-[0.05em] uppercase text-black/40 dark:text-white/50 mb-8">
                Sensitivity Analysis
              </h3>
              ...
              <div className="space-y-3">
                {assumptions.map((assumption, index) => (
                  <div key={index} className="flex items-start gap-3 py-2">
                    <div className="w-1.5 h-1.5 rounded-full bg-black/70 dark:bg-white/70 mt-2 flex-shrink-0" />
                    <p className="text-small text-black/70 dark:text-white/70 leading-relaxed">{assumption}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Risk Breakdown */}
          <div className="bg-white dark:bg-[#0a0a0a] border border-black/5 dark:border-white/10 rounded-lg p-10 mb-6">
            <h3 className="text-small tracking-[0.05em] uppercase text-black/40 dark:text-white/50 mb-8 flex items-center gap-2">
              ...
              {risks.map((risk, index) => (
                <div key={index}>
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      <div className="text-small text-black/95 dark:text-white mb-1">{risk.category}</div>
                      <div className="text-caption text-black/50 dark:text-white/50">{risk.mitigation}</div>
                    </div>
                    <div className="text-small text-black/70 dark:text-white/70">{risk.score}%</div>
                  </div>
            ...
            <div className="space-y-4">
              {milestones.map((milestone, index) => (
                <div key={index} className="flex items-center gap-6 py-3">
                  <div className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center text-caption text-black/50 dark:text-white/50 flex-shrink-0">
                    {index + 1}
                  </div>
                  <div className="flex-1">
                    <div className="text-small text-black/95 dark:text-white mb-1">{milestone.phase}</div>
                    <div className="text-caption text-black/40 dark:text-white/40">{milestone.duration}</div>
                  </div>
                  <div className="h-1 w-32 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden">
                    <div className={`h-full ${milestone.status === 'completed' ? 'bg-black dark:bg-white w-full' : 'bg-black dark:bg-white w-0'}`} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-blue-500/5 border border-blue-500/10 rounded-lg p-10">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h3 className="text-body tracking-tight text-black/95 dark:text-white mb-3">
                  Why This Strategy?
                </h3>
                ...
                <ul className="space-y-2">
                  <li className="text-small text-black/60 dark:text-white/60 leading-relaxed flex items-start gap-2">
