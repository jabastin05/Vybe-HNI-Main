...
export function ServiceCard({
  ...
  return (
    <div
      onClick={onClick}
      className={`
        relative bg-white/95 dark:bg-[#1A1A1A]/95 backdrop-blur-[40px] rounded-xl p-4 md:p-5 cursor-pointer
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 md:gap-3 mb-4 md:mb-5 relative z-10">
        <div className="flex-1 min-w-0">
          {categoryName && (
            <div className="text-caption font-bold tracking-[0.05em] uppercase text-black/40 dark:text-white/50 mb-1.5">
              {categoryName}
            </div>
          )}
        ...
        {badge && (
          <div className="flex-shrink-0 px-3 py-1.5 bg-amber-500/20 rounded-lg border border-amber-500/30">
            <span className="text-caption font-bold tracking-[0.05em] uppercase text-amber-600 dark:text-amber-400 whitespace-nowrap">
              {badge}
            </span>
          </div>
        )}
      ...
      {/* Description */}
      <p className="text-small text-black/60 dark:text-white/60 leading-relaxed mb-4 md:mb-5">
        {description}
      </p>
      ...
      {primaryAttribute && (
        <div className="flex items-center gap-2 md:gap-3 mb-4 md:mb-5 flex-wrap">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-blue-500/10 dark:bg-blue-500/20 rounded-lg border border-blue-500/20">
            ...
            <span className="text-caption font-bold tracking-[0.05em] uppercase text-blue-700 dark:text-blue-400">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-emerald-500/10 dark:bg-emerald-500/20 rounded-lg border border-emerald-500/20">
            ...
            <span className="text-caption font-bold tracking-[0.05em] uppercase text-emerald-700 dark:text-emerald-400">
      ...
      {primaryAttribute && primaryAttribute.deliverables && primaryAttribute.deliverables.length > 0 && (
        <div className="space-y-1.5 md:space-y-2 mb-4 md:mb-5 pb-4 md:pb-5 border-b border-black/5 dark:border-white/10 flex-1">
          {primaryAttribute.deliverables.slice(0, 2).map((deliverable, index) => (
            <div key={index} className="flex items-start gap-2">
              ...
              <span className="text-small text-black/70 dark:text-white/70 font-medium">
                {deliverable}
              </span>
            </div>
          ))}
      ...
      {hasMultipleOptions && (
        <div className="mb-4 md:mb-5 pb-4 md:pb-5 border-b border-black/5 dark:border-white/10">
          <span className="text-small font-medium text-emerald-600 dark:text-emerald-400">
            <span className="font-bold">{attributes.length}</span> options available
          </span>
        </div>
      )}
      ...
      <div className="mt-auto">
        <Link
          ...
          className="flex items-center justify-center gap-2 text-small font-medium
