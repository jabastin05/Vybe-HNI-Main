  ...
  return (
    <>
      ...
      >
        ...
        <span className="text-[12px] md:text-[13px] font-medium text-emerald-700 dark:text-emerald-300 hidden sm:inline">
          Request a Call
        </span>
        <span className="text-[12px] font-medium text-emerald-700 dark:text-emerald-300 sm:hidden">
          Call
        </span>
      </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-[#1A1A1A] rounded-[24px] border border-black/10 dark:border-white/10
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-3 p-4 bg-emerald-500/5 border border-emerald-500/20 rounded-xl">
                ...
                <div className="flex-1">
                  <div className="text-[14px] font-medium text-black dark:text-white leading-tight mb-0.5">
                    {rm.name}
                  </div>
                  <div className="text-[12px] text-black/60 dark:text-white/60 leading-tight">
              ...
              <div>
                <label className="text-[12px] font-medium text-black/70 dark:text-white/70 mb-2 block uppercase tracking-wide">
                  Preferred Availability <span className="text-black/40 dark:text-white/40 normal-case">(Optional)</span>
                </label>
                
                <div className="grid grid-cols-2 gap-3">
                  {/* Date Picker */}
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-black/40 dark:text-white/40" />
                    <input
                      type="date"
                      value={selectedDate}
                      onChange={(e) => setSelectedDate(e.target.value)}
                      min={new Date().toISOString().split('T')[0]}
                      className="w-full pl-10 pr-3 py-2.5 bg-white dark:bg-[#111111] border border-black/10 dark:border-white/10
                                 rounded-xl text-[13px] text-black dark:text-white
                  ...
                  {/* Time Picker */}
                  <div className="relative">
                    <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-black/40 dark:text-white/40" />
                    <input
                      type="time"
                      value={selectedTime}
              ...
              <div>
                <label className="text-[12px] font-medium text-black/70 dark:text-white/70 mb-2 block uppercase tracking-wide">
                  Notes <span className="text-black/40 dark:text-white/40 normal-case">(Optional)</span>
                </label>
                <textarea
                  value={notes}
                  ...
                  rows={3}
                  className="w-full px-4 py-3 bg-white dark:bg-[#111111] border border-black/10 dark:border-white/10
                             rounded-xl text-[13px] text-black dark:text-white placeholder:text-black/40 dark:placeholder:text-white/40
              ...
              <div className="p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                <p className="text-[12px] text-black/70 dark:text-white/70 leading-relaxed">
            ...
            {/* Modal Footer */}
            <div className="flex items-center gap-3 p-6 border-t border-black/5 dark:border-white/5">
              <button
                onClick={() => setIsModalOpen(false)}
                className="flex-1 px-4 py-2.5 bg-white dark:bg-[#111111] border border-black/10 dark:border-white/10
                           rounded-xl text-[14px] font-medium text-black dark:text-white
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-gradient-to-br from-emerald-500 to-emerald-600
                           rounded-xl text-[14px] font-medium text-white
